...
import itertools
import time
from thresholds import rejection_score, thr_output, thr_diff, thr_ratio
...
def calc_roc_multiclass(outputs_true, outputs_pred, n_classes,
                        outputs_outl=None, threshold = 0.9):
    ...
    for i in range(n_classes):
        if len(y_true_classes[i]) > 1 and calc_recall(y_true_classes[i]) <= threshold:
