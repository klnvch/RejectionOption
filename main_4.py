...
FILE_TEST = [
    'sigmoid_gradient_0.1_20000_256_[24]_1481742134',
    ...
    'sigmoid_adam_0.01_2000_128_[20, 20]_1482340044',
...
if __name__ == '__main__':
    ...
    print('#Rejection Option', file=file)
    print('', file=file)
    ...
    for test in FILE_TEST:
        with open(DIR_TEST + test + '.csv', 'r') as f:
            ...
            print('| {:s} | {:6d} | {:8f} | {:8f} | {:8f} | {:8f} | {:8f} | {:8f} | {:4d} |'
              .format(test, data[-1,0].astype(np.int), data[-1,1].astype(np.float), data[-1,2].astype(np.float),
                      data[-1,3].astype(np.float), data[-1,4].astype(np.float), data[-1,5].astype(np.float),
                      data[-1,6].astype(np.float), int(np.sum(data[1:,7].astype(np.float)) / 60.0)), file=file)
    ...
    print('', file=file)
    for test in FILE_TEST:
        print('![](images/' + test + '.png)', file=file)
    file.close()
