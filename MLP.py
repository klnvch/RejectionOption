...
The code is imperfect and can be replaced with Keras library
'''
...
class MLP:
    ...
    def __init__(self, learning_rate, layers, functions,
        ...
        if 'rbf' in functions or 'rbf-reg' in functions:
            ...
            h, _ = self.add_layer(x, [7 * 7 * 64, self.n_hidden[0]],
            ...
            y, r1 = self.add_layer(self.x, [self.n_input, self.n_output],
                                   None, '', None)
            regularizers = r1
            
        elif len(self.n_hidden) == 1:
            h, r1 = self.add_layer(self.x, [self.n_input, self.n_hidden[0]],
            ...
            h1, r1 = self.add_layer(self.x, [self.n_input, self.n_hidden[0]],
                                    functions[0], '1', self.keep_prob)
            h2, r2 = self.add_layer(h1, [self.n_hidden[0], self.n_hidden[1]],
                                    functions[1], '2', self.keep_prob)
            y, r3 = self.add_layer(h2, [self.n_hidden[1], self.n_output],
                                   None, '3', None)
            ...
            h2, r2 = self.add_layer(h1, [self.n_hidden[0], self.n_hidden[1]],
                                    functions[1], '2', self.keep_prob)
            h3, r3 = self.add_layer(h2, [self.n_hidden[1], self.n_hidden[2]],
                                    functions[2], '3', self.keep_prob)
            y, r4 = self.add_layer(h3, [self.n_hidden[2], self.n_output],
                                   None, '4', None)
    ...
    def add_layer(self, x, shape, activation_function, postfix_name,
        ...
        w = tf.Variable(tf.truncated_normal(shape, stddev=0.1),
                        name='w_' + postfix_name)
        b = tf.Variable(tf.constant(0.1, shape=[shape[1]]),
                        name='b_' + postfix_name)
        if regularization is None:  l2_loss = None
        else:                       l2_loss = regularization(w)
        if activation_function is None:
            return tf.matmul(x, w) + b, l2_loss
        else:
            y = activation_function(tf.matmul(x, w) + b)
            if keep_prob is not None:
                layer = tf.nn.dropout(y, keep_prob)
            return layer, l2_loss
    
    def add_rbf(self, x, n_input, n_centers):
        self.centers = tf.Variable(
            tf.random_uniform([n_centers, n_input], -1.0, 1.0), name='centers')
        self.variances = tf.Variable(
            tf.truncated_normal([n_centers], 15.0, 5.0), name='widths')
    ...
    def add_rbf_reg(self, x, n_input, n_centers):
        self.centers = tf.Variable(
            tf.random_uniform([n_centers, n_input], -1.0, 1.0), name='centers')
        self.variances = tf.Variable(
            tf.truncated_normal([n_centers, n_input], 15.0, 5.0), name='widths')
    ...
    def add_conv(self, x):
        ...
        u_conv1 = tf.nn.conv2d(x, W_conv1,
                               strides=[1, 1, 1, 1], padding='SAME')
        h_conv1 = tf.nn.relu(u_conv1 + b_conv1)
        h_pool1 = tf.nn.max_pool(h_conv1, ksize=[1, 2, 2, 1],
                                 strides=[1, 2, 2, 1], padding='SAME')
        # The second convolutional layer
        W_conv2 = tf.Variable(tf.truncated_normal([5, 5, 32, 64], stddev=0.1))
        b_conv2 = tf.Variable(tf.constant(0.1, shape=[64]))
        u_conv2 = tf.nn.conv2d(h_pool1, W_conv2,
                               strides=[1, 1, 1, 1], padding='SAME')
        h_conv2 = tf.nn.relu(u_conv2 + b_conv2)
